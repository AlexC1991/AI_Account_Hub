...
class AccountCalendarApp(tk.Tk):
    ...
    def coding_selected_profile(self) -> dict | None:
        for profile in self.profiles:
            if profile_id(profile) == self.coding_profile_id:
                return profile
        return None

    def _sync_coding_profile_combo(self) -> None:
        options = self.coding_profile_options()
        self.coding_profile_option_map = options
        available_ids = set(options.values())
        if self.coding_profile_id not in available_ids:
            ready = next((profile for profile in self.profiles if effective_state(profile) == "ready"), None)
            self.coding_profile_id = profile_id(ready) if ready is not None else (next(iter(available_ids), ""))
        selected_label = next((label for label, pid in options.items() if pid == self.coding_profile_id), "")
        if hasattr(self, "coding_profile_combo"):
            self.coding_profile_combo.configure(values=list(options))
        self.coding_profile_var.set(selected_label)

    def _coding_control_preferences(self, profile: dict) -> dict[str, str]:
        controls = self.settings.get("codingControls")
        saved_controls = controls if isinstance(controls, dict) else {}
        saved = saved_controls.get(profile_id(profile))
        pid = profile_id(profile)
        if pid not in self._coding_defaults_cache:
            self._coding_defaults_cache[pid] = read_coding_profile_defaults(profile)
        defaults = self._coding_defaults_cache[pid]
        result = {
    ...
    def _save_coding_control_preference(self, profile: dict, key: str, value: str) -> None:
        ...
        if not isinstance(controls, dict):
            controls = {}
            self.settings["codingControls"] = controls
    ...
    def _coding_model_rows(self, profile: dict) -> list[tuple[str, str]]:
        provider = provider_key(profile)
        if self.native_models_profile_id == profile_id(profile) and self.native_models:
    ...
    def _coding_effort_rows(self, profile: dict, model_value: str) -> list[tuple[str, str]]:
        provider = provider_key(profile)
        if provider == "codex" and self.native_models_profile_id == profile_id(profile):
            model = next(
                (
                    item
                    for item in self.native_models
    ...
    def _sync_coding_controls(self) -> None:
        ...
        if profile is None:
            return
        preferences = self._coding_control_preferences(profile)
        model_rows = self._coding_model_rows(profile)
        model_values = {value for _label, value in model_rows}
        model_value = preferences["model"]
        if not model_value and self.native_models_profile_id == profile_id(profile):
            default_model = next(
                (
        ...
        self.coding_model_options = dict(model_rows)
        self.coding_model_var.set(self._coding_option_label(self.coding_model_options, model_value))

        effort_rows = self._coding_effort_rows(profile, model_value)
        ...
        self.coding_effort_options = dict(effort_rows)
        self.coding_effort_var.set(self._coding_option_label(self.coding_effort_options, effort_value))
        ...
        self.coding_personality_options = dict(personality_rows)
        self.coding_personality_var.set(self._coding_option_label(self.coding_personality_options, personality_value))
        ...
        self.coding_access_options = dict(access_rows)
        self.coding_access_var.set(self._coding_option_label(self.coding_access_options, access_value))

        if hasattr(self, "coding_model_combo"):
            self.coding_model_combo.configure(values=list(self.coding_model_options), state="readonly")
            effort_state = "readonly" if len(self.coding_effort_options) > 1 else "disabled"
            self.coding_effort_combo.configure(values=list(self.coding_effort_options), state=effort_state)
            personality_state = "readonly" if len(self.coding_personality_options) > 1 else "disabled"
            self.coding_personality_combo.configure(values=list(self.coding_personality_options), state=personality_state)
            self.coding_access_combo.configure(values=list(self.coding_access_options), state="readonly")

    def coding_control_values(self) -> dict[str, str]:
        return {
            "model": self.coding_model_options.get(self.coding_model_var.get(), ""),
            "effort": self.coding_effort_options.get(self.coding_effort_var.get(), ""),
            "personality": self.coding_personality_options.get(self.coding_personality_var.get(), ""),
            "access": self.coding_access_options.get(self.coding_access_var.get(), "default"),
        }
    ...
    def on_coding_control_changed(self, key: str) -> None:
        ...
        if profile is None or key not in {"model", "effort", "access", "personality"}:
            return
        values = self.coding_control_values()
        self._save_coding_control_preference(profile, key, values[key])
        if key == "model":
            self._sync_coding_controls()
        self.coding_composer_status.configure(text="Applies next turn")
    ...
    def _set_coding_control_value(self, key: str, value: str) -> bool:
        ...
        if value not in values_by_key.get(key, set()):
            return False
        self._save_coding_control_preference(profile, key, value)
        self._sync_coding_controls()
        return True
    ...
    def _effective_codex_model(self, profile: dict, controls: dict[str, str]) -> str:
        ...
        defaults = self._coding_defaults_cache.get(profile_id(profile)) or read_coding_profile_defaults(profile)
        if defaults.get("model"):
            return str(defaults["model"])
        for _label, value in self._coding_model_rows(profile):
            if value:
                return value
        return "gpt-5.5"

    def _codex_plan_collaboration_mode(self, profile: dict, controls: dict[str, str]) -> dict:
        settings: dict[str, object] = {"model": self._effective_codex_model(profile, controls)}
    ...
    def set_coding_context_tab(self, tab: str) -> None:
        if tab not in {"session", "skills", "files", "terminal"}:
            return
        self.coding_context_tab = tab
        if tab == "skills":
            self.refresh_native_skills()
        self._render_coding_context()
        self._update_coding_details_button()

    def _render_coding(self) -> None:
        if not hasattr(self, "coding_project_scroll"):
            return
        self._sync_coding_profile_combo()
        self._sync_coding_controls()
        choices = self.coding_workspace_choices()
        if self.coding_workspace_var.get() not in choices:
            self.coding_workspace_var.set(choices[0])
        workspace = self.coding_workspace_var.get()
        profile = self.coding_selected_profile()
        project_name = Path(workspace).name or workspace
        thread_title = self.native_thread_title or project_name
        self.coding_title.configure(text="New thread" if not self.native_thread_id else thread_title)
        if profile is None:
            self.coding_subtitle.configure(text=project_name)
        else:
            self.coding_subtitle.configure(text=f"{project_name} | {provider_label(profile)} | {profile.get('name', 'Account')}")

        self._render_coding_projects()
        self._render_coding_sidebar_account()
        self._render_coding_stream()
        self._render_coding_context()
        if self.coding_details_visible:
            self.coding_inspector.grid(row=0, column=2, sticky="nsew")
            ...
            self.coding_inspector.grid_remove()
        new_thread_state = "normal" if profile is not None and not self.busy and not self.native_busy else "disabled"
        self.coding_new_thread_button.configure(state=new_thread_state)
        self.coding_sidebar_new_thread_button.configure(state=new_thread_state)
        self.coding_sidebar_accounts_button.configure(text=f"Accounts    {len(self.profiles)}")
        send_available = profile is not None and not self.busy and not self.native_busy
        self.coding_send_button.configure(state="normal" if send_available else "disabled")
        self.coding_stop_button.configure(state="normal" if self.native_busy else "disabled")
        self.coding_attach_button.configure(state="normal" if profile is not None and not self.native_busy else "disabled")
        controls_state = "disabled" if profile is None or self.native_busy else "readonly"
        self.coding_model_combo.configure(state=controls_state)
        effort_state = controls_state if len(self.coding_effort_options) > 1 else "disabled"
        self.coding_effort_combo.configure(state=effort_state)
        self.coding_access_combo.configure(state=controls_state)
        self._render_native_attachments()
        self._render_coding_short_limit()
        if self.native_busy:
            self.coding_composer_status.configure(text=f"{provider_label(profile)} working" if profile else "Working")
        elif self.native_attachments:
            self.coding_composer_status.configure(text=native_attachment_status_text(self.native_attachments))
        elif self.coding_session_active:
            session = self.native_thread_id[:8] if self.native_thread_id else "new"
            self.coding_composer_status.configure(text=f"Native session {session}")
        elif not self.native_attachments:
            self.coding_composer_status.configure(text="Native passthrough")
        self._update_coding_details_button()
        self._update_status_context()

    def _render_coding_short_limit(self) -> None:
        if not hasattr(self, "coding_short_limit_label"):
            return
        coding = coding_palette(self.theme_name)
        profile = self.coding_selected_profile()
        ...
        self.coding_short_limit_label.configure(text=label, fg=color if left is not None else coding["muted"])
        meter = self.coding_short_limit_meter
        meter.delete("all")
    ...
    def _render_coding_projects(self) -> None:
        if not hasattr(self, "coding_project_scroll"):
            return
        coding = coding_palette(self.theme_name)
        self._clear(self.coding_project_scroll.inner)
        term = self._coding_search_term()
        workspaces = [
            workspace
            for workspace in self.coding_workspace_choices()
            if not term or term in workspace.lower() or term in (Path(workspace).name or workspace).lower()
        ]
        self.coding_project_count.configure(text=str(len(workspaces)))
        selected_workspace = self.coding_workspace_var.get()
        for workspace in workspaces:
            ...
            row = tk.Frame(
                self.coding_project_scroll.inner,
                bg=bg,
                cursor="hand2",
            ...
            count_text = "..." if selected and self.native_loading_threads else str(len(project_threads))
            ...
            callback = lambda _event, value=workspace: self.select_coding_workspace(value)
            self._bind_recursive(row, callback)
            if project_threads:
                ...
                expansion_key = self._coding_project_thread_expansion_key(workspace)
                expanded = expansion_key in self.expanded_coding_project_threads
                ...
                for index, thread in enumerate(project_threads[:visible_thread_count], start=1):
                    thread_id = str(thread.get("id") or "")
                    thread_selected = bool(thread_id and thread_id == self.native_thread_id)
                    ...
                    callback_thread = lambda _event, item=thread: self.select_native_thread(item)
                    self._bind_recursive(thread_row, callback_thread)
                ...
                if len(project_threads) > visible_thread_count or (
                    ...
                    more = tk.Button(
                        row,
                        text=label,
                        command=lambda value=workspace: self.toggle_coding_project_thread_expansion(value),
                        bg=bg,
                        fg=coding["muted"],
                        activebackground=coding["active"],
                        activeforeground=coding["ink"],
            ...
            elif selected and not self.native_loading_threads:
                tk.Label(
                    row,
    ...
    def _render_coding_sidebar_account(self) -> None:
        if not hasattr(self, "coding_sidebar_account"):
            return
        coding = coding_palette(self.theme_name)
        self._clear(self.coding_sidebar_account)
        profile = self.coding_selected_profile()
        if profile is None:
            tk.Label(
                self.coding_sidebar_account,
            ...
            return
        row = tk.Frame(self.coding_sidebar_account, bg=coding["rail"])
        row.pack(fill="x", padx=12, pady=10)
        self._service_icon(row, provider_key(profile), size=28).pack(side="left", padx=(0, 8))
        ...
        selector = tk.Frame(self.coding_sidebar_account, bg=coding["rail"])
        ...
        self.coding_profile_combo = ttk.Combobox(
            selector,
            textvariable=self.coding_profile_var,
            state="readonly",
            values=list(getattr(self, "coding_profile_option_map", {})),
            style="Coding.TCombobox",
        )
        self.coding_profile_combo.pack(fill="x")
        self.coding_profile_combo.bind("<<ComboboxSelected>>", lambda _event: self.on_coding_profile_changed())

    def _configure_coding_stream_tags(self) -> None:
        coding = coding_palette(self.theme_name)
        stream = self.coding_stream_text
        width = max(720, stream.winfo_width())
        margin = max(42, (width - 820) // 2)
        self._coding_stream_margin = margin
        base = {
            "foreground": coding["ink"],
            "font": ("Segoe UI", 10),
            "lmargin1": margin,
        ...
        for match in token_pattern.finditer(text):
            ...
            if token.startswith("`"):
                ...
                if link is None:
                    ...
                    self._coding_link_counter += 1
                    link_tag = f"coding_link_{self._coding_link_counter}"
                    ...
                    stream.tag_bind(link_tag, "<Button-1>", lambda _event, value=target: self._open_coding_link(value))
        ...
        for raw_line in value.replace("\r\n", "\n").split("\n"):
            ...
            if line_image_refs and not in_code:
                ...
                if text_without_images:
                    self._insert_coding_inline(text_without_images, base_tag)
                    stream.insert("end", "\n", (base_tag,))
                for ref in line_image_refs:
                    self._insert_coding_image_card(ref)
                continue
            ...
            if heading:
                ...
                self._insert_coding_inline(heading.group(2), tag)
                stream.insert("end", "\n", (tag,))
            elif bullet:
                stream.insert("end", "\u2022\t", ("md_list",))
                self._insert_coding_inline(bullet.group(1), "md_list")
                stream.insert("end", "\n", ("md_list",))
            elif numbered:
                stream.insert("end", f"{numbered.group(1)}.\t", ("md_list",))
                self._insert_coding_inline(numbered.group(2), "md_list")
                stream.insert("end", "\n", ("md_list",))
            elif quote:
                self._insert_coding_inline(quote.group(1), "md_quote")
                stream.insert("end", "\n", ("md_quote",))
            elif re.match(r"^\s*([-*_])\1\1+\s*$", raw_line):
                stream.insert("end", "\n", ("turn_meta",))
            elif raw_line:
                self._insert_coding_inline(raw_line, base_tag)
                stream.insert("end", "\n", (base_tag,))
            else:
                stream.insert("end", "\n", (base_tag,))
        if in_code:
    ...
    def _open_coding_link(self, target: str) -> None:
        ...
        if path.exists():
            try:
                ...
                self.status_var.set(f"Could not open link: {error}")
    ...
    def _build_image_preview(self, master: tk.Misc, ref: dict, bg: str, max_width: int = 520, max_height: int = 260) -> tk.Frame:
        ...
        image = self._load_coding_image(ref, max_width=max_width, max_height=max_height)
        ...
        tk.Label(
            info,
            text=clip_text(self._image_ref_label(ref), 60),
            bg=bg,
            fg=coding["ink"],
            font=("Segoe UI", 8, "bold"),
            anchor="w",
        ...
        if target:
            tk.Button(
                info,
                text="Open",
                command=lambda value=target: self._open_coding_link(value),
                bg=coding["panel"],
                fg=coding["ink"],
                activebackground=coding["active"],
                activeforeground=coding["ink"],
    ...
    def _insert_coding_image_card(self, ref: dict, title: str = "Image") -> None:
        ...
        self._build_image_preview(card, ref, coding["panel"], max_width=500, max_height=240).pack(fill="x", pady=(6, 0))
    ...
    def _insert_coding_user_message(self, value: str, message: dict | None = None) -> None:
        ...
        if message:
            ...
            for ref in self._coding_image_refs(message, ""):
                key = str(ref.get("path") or ref.get("url") or ref.get("data") or ref.get("name") or "").lower()
                if key and key not in existing:
        ...
        for attachment in attachments[:6]:
            name = self._image_ref_label(attachment)
            ...
            if clean_windows_path_text(attachment.get("path")) or attachment.get("data"):
                self._build_image_preview(bubble, attachment, coding["active"], max_width=420, max_height=160).pack(fill="x", pady=(0, 6))
        if text:
            display = re.sub(r"\*\*([^*\n]+)\*\*", r"\1", text)
            display = re.sub(r"`([^`\n]+)`", r"\1", display)
            tk.Label(
    ...
    def _insert_coding_activity_card(self, message: dict, value: str) -> None:
        ...
        kind = str(message.get("kind") or self._activity_kind_from_text(value))
        ...
        if kind in {"diff", "file_change"}:
            diff = str(message.get("diff") or self.native_turn_diff or value).strip()
            ...
            if diff:
                ...
                self._insert_diff_text(diff_box, compact_history_text(diff, limit=7000))
                diff_box.configure(state="disabled")
            tk.Button(
                card,
                text="Open Files View",
                command=lambda: self.set_coding_context_tab("files"),
                bg=coding["active"],
                fg=coding["ink"],
            ...
            if not self._insert_plan_rows(card, message, value, coding["panel"]):
            ...
            refs = self._coding_image_refs(message, value)
            if refs:
                for ref in refs[:4]:
                    self._build_image_preview(card, ref, coding["panel"], max_width=500, max_height=220).pack(fill="x", pady=(7, 0))
            else:
                tk.Label(card, text=clip_text(coding_display_text(value), 180), bg=coding["panel"], fg=coding["muted"], font=("Segoe UI", 8), wraplength=620, justify="left", anchor="w").pack(fill="x", pady=(7, 0))
        else:
            body = value
    ...
    def _plain_coding_activity_text(self, message: dict | None, value: str) -> str:
        ...
        if kind in {"diff", "file_change"}:
            ...
            diff = str(message.get("diff") or self.native_turn_diff or "").strip()
        ...
        if kind == "image":
            refs = self._coding_image_refs(message, value)
            labels = ", ".join(clip_text(self._image_ref_label(ref), 44) for ref in refs[:4])
    ...
    def _insert_coding_activity(self, role: str, value: str, message: dict | None = None) -> None:
        ...
        if role == "error":
            ...
            return
        if message and self._coding_message_uses_windows(message, value):
            self._insert_coding_activity_card(message, value)
            return
        if message:
            body = self._plain_coding_activity_text(message, value)
            if body:
    ...
    def _coding_message_uses_windows(self, message: dict, text: str) -> bool:
        ...
        if role == "user":
            return True
        if role == "assistant":
            return bool(self._coding_image_refs(message, text) or markdown_image_refs(text))
        if role == "activity" and self.native_busy:
            return False
    ...
    def _coding_stream_state(self) -> tuple[tuple[str, ...], dict[str, str], dict[str, int]]:
        ...
        for index, message in enumerate(self.native_messages):
            role = str(message.get("role") or "activity")
            text = coding_display_text(message.get("text") or "")
            key = self._coding_stream_message_key(index, message)
            uses_windows = self._coding_message_uses_windows(message, text)
    ...
    def _try_update_coding_stream_delta(self) -> bool:
        if not self._coding_stream_signature or not self._coding_stream_message_ranges:
            return False
        signature, texts, indexes = self._coding_stream_state()
        if signature != self._coding_stream_signature:
            return False
        changed = [
            key
            for key, text in texts.items()
            if text != getattr(self, "_coding_stream_last_texts", {}).get(key)
        ]
        if len(changed) != 1:
            return False
        key = changed[0]
        index = indexes.get(key)
        if index is None or index >= len(self.native_messages):
            return False
        message = self.native_messages[index]
        text = texts[key]
        role = str(message.get("role") or "")
        if role not in {"assistant", "activity"} or self._coding_message_uses_windows(message, text):
            return False
        marks = self._coding_stream_message_ranges.get(key)
        if marks is None:
            return False
        stream = self.coding_stream_text
        ...
        try:
            ...
            if role == "activity":
                body = self._plain_coding_activity_text(message, text)
            ...
            self._coding_stream_last_texts = texts
            if self._coding_scroll_after_id:
                try:
                    self.after_cancel(self._coding_scroll_after_id)
                except tk.TclError:
                    pass
            self._coding_scroll_after_id = self.after(60, self._scroll_coding_to_bottom)
            return True
        except tk.TclError:
            try:
                stream.configure(state="disabled")
    ...
    def _try_append_coding_stream_messages(self) -> bool:
        if not self._coding_stream_signature:
            return False
        signature, texts, _indexes = self._coding_stream_state()
        old_count = len(self._coding_stream_signature)
        if len(signature) <= old_count or tuple(signature[:old_count]) != self._coding_stream_signature:
            return False
        appended_indexes = range(old_count, len(signature))
        for index in appended_indexes:
            if index >= len(self.native_messages):
                return False
            message = self.native_messages[index]
            text = texts.get(self._coding_stream_message_key(index, message), "")
            if str(message.get("role") or "") == "user" or self._coding_message_uses_windows(message, text):
                return False
        ...
        try:
            ...
            for index in appended_indexes:
                ...
                if role == "assistant":
                    self._insert_coding_markdown(text, muted=bool(message.get("muted")))
                else:
                    self._insert_coding_activity(role, text, message=message)
                stream.mark_set(end_mark, "end-1c")
                stream.mark_gravity(end_mark, "right")
                self._coding_stream_message_ranges[key] = (start_mark, end_mark)
            stream.configure(state="disabled")
            self._coding_stream_signature = signature
            self._coding_stream_last_texts = texts
            if self._coding_scroll_after_id:
                try:
                    self.after_cancel(self._coding_scroll_after_id)
    ...
    def _render_coding_stream(self) -> None:
        if not hasattr(self, "coding_stream_text"):
            return
        if self._try_update_coding_stream_delta():
            return
        if self._try_append_coding_stream_messages():
            return
        stream = self.coding_stream_text
        stream.configure(state="normal")
        for child in stream.winfo_children():
            child.destroy()
        self.image_refs = []
        self._coding_stream_message_ranges = {}
        stream.delete("1.0", "end")
        self._coding_link_counter = 0
        self._configure_coding_stream_tags()
        if not self.native_messages:
            profile = self.coding_selected_profile()
            workspace = Path(self.coding_workspace_var.get() or DEFAULT_WORKSPACE).name
            ...
            stream.configure(state="disabled")
            self._coding_stream_signature = ()
            self._coding_stream_last_texts = {}
            return
        ...
        for index, message in enumerate(self.native_messages):
            ...
            if not text:
                continue
            if role == "user":
                self._insert_coding_user_message(text, message=message)
            elif role == "assistant":
                key = self._coding_stream_message_key(index, message)
                if not self._coding_message_uses_windows(message, text):
                    start_mark = f"{key}_start"
                    end_mark = f"{key}_end"
                    stream.mark_set(start_mark, "end-1c")
                    stream.mark_gravity(start_mark, "left")
                ...
                key = self._coding_stream_message_key(index, message)
                if role == "activity" and not self._coding_message_uses_windows(message, text):
                    start_mark = f"{key}_start"
                    end_mark = f"{key}_end"
                    stream.mark_set(start_mark, "end-1c")
                    stream.mark_gravity(start_mark, "left")
    ...
    def _scroll_coding_to_bottom(self) -> None:
        self._coding_scroll_after_id = None
        if self._closing:
            return
        try:
            self.coding_stream_text.yview_moveto(1.0)
        except tk.TclError:
            pass

    def _render_coding_context(self) -> None:
        coding = coding_palette(self.theme_name)
        for tab, button in getattr(self, "coding_context_buttons", {}).items():
            selected = tab == self.coding_context_tab
            button.configure(
        ...
        if not hasattr(self, "coding_context_scroll"):
            return
        body = self.coding_context_scroll.inner
        self._clear(body)
        if self.coding_context_tab == "files":
            self._render_coding_files(body)
        elif self.coding_context_tab == "skills":
            self._render_coding_skills(body)
        elif self.coding_context_tab == "terminal":
            self._render_coding_terminal(body)
        else:
            self._render_coding_session(body)
    ...
    def _render_coding_session(self, master: tk.Misc) -> None:
        ...
        self._service_icon(head, provider_key(profile), size=34).pack(side="left", padx=(0, 9))
        ...
        state = self.profile_state(profile)
        self._status_badge(head, status_badge_text(profile, state), state).pack(side="right")

        weekly_left = percent_left(profile.get("weeklyLimitUsedPercent"))
        short_left = percent_left(profile.get("shortLimitUsedPercent"))
        provider = provider_key(profile)
        transport_state = "Running" if self.native_busy else "Connected" if self.coding_session_active else "Idle"
        if provider == "cursor" and not self.cursor_agent_path:
        ...
        fields = [
            ...
            ("Style", self.coding_personality_var.get() or controls["personality"] or "Provider default"),
            ("Access", self.coding_access_var.get() or controls["access"]),
            ("Weekly left", format_percent(weekly_left)),
            ("Session left", format_percent(short_left)),
            ("Transport", transport_state),
            ("Session", self.native_thread_id[:12] if self.native_thread_id else "-"),
            ("Tokens", native_token_usage_label(self.native_token_usage)),
            ("Project", clip_text(Path(self.coding_workspace_var.get()).name, 24)),
    ...
    def _native_skill_rows(self) -> tuple[list[dict], list[str]]:
        rows: list[dict] = []
        errors: list[str] = []
        for entry in self.native_skills:
            if not isinstance(entry, dict):
                continue
    ...
    def _render_coding_skills(self, master: tk.Misc) -> None:
        ...
        refresh = tk.Button(
            header,
            text="Refresh",
            command=lambda: self.refresh_native_skills(force=True),
            bg=coding["panel"],
            fg=coding["ink"],
            activebackground=coding["active"],
            activeforeground=coding["ink"],
        ...
        if profile is None:
            ...
            return
        if provider_key(profile) != "codex":
            text = self.native_skills_error or f"{provider_label(profile)} does not expose Codex app-server skills."
            ...
            return
        if self.native_skills_loading:
            tk.Label(master, text="Loading skills...", bg=coding["rail"], fg=coding["muted"], font=("Segoe UI", 8), anchor="w").pack(fill="x", pady=(0, 8))
        if self.native_skills_error:
            tk.Label(master, text=self.native_skills_error, bg=coding["rail"], fg=RED, font=("Segoe UI", 8), wraplength=285, justify="left", anchor="w").pack(fill="x", pady=(0, 8))

        rows, errors = self._native_skill_rows()
        ...
        for skill in rows[:80]:
            ...
            self._status_badge(top, "Enabled" if skill_enabled else "Disabled", "ready" if skill_enabled else "idle").pack(side="right")
            meta = f"{scope}"
            if path:
            ...
            tk.Button(
                ...
                command=lambda item=skill, state=not skill_enabled: self.set_native_skill_enabled(item, state),
                bg=coding["active"],
                fg=coding["ink"],
                activebackground=coding["composer"],
                activeforeground=coding["ink"],
    ...
    def _render_coding_files(self, master: tk.Misc) -> None:
        ...
        if self.native_file_changes or self.native_turn_diff:
            ...
            if self.native_turn_diff:
                ...
                diff_box.insert("1.0", self.native_turn_diff)
                diff_box.configure(state="disabled")
    ...
    def _render_coding_terminal(self, master: tk.Misc) -> None:
        ...
        state = "Native process attached" if self.native_transport is not None else "No native process attached"
        tk.Label(terminal, text=state, bg=DARK, fg=MUTED, font=("Consolas", 8)).pack(anchor="w", pady=(8, 0))
        for line in self.native_diagnostics[-20:]:
            tk.Label(
                terminal,
    ...
    def visible_profiles(self) -> list[dict]:
        ...
        if not term:
            ...
            for profile in self.profiles:
                ...
                if term in haystack:
                    rows.append(profile)
        return self.sorted_profiles(rows)

    def sorted_profiles(self, profiles: list[dict]) -> list[dict]:
        mode = self.sort_var.get() if hasattr(self, "sort_var") else "Manual"
        if mode not in SORT_CHOICES or mode == "Manual":
            return profiles
        indexed = {profile_id(profile): index for index, profile in enumerate(self.profiles)}
        ...
        def key(profile: dict) -> tuple:
            ...
            if mode == "State":
                ...
                return (order.get(self.profile_state(profile), 9), str(profile.get("name", "")).lower(), indexed.get(pid, 0))
    ...
    def profile_state(self, profile: dict) -> str:
        pid = profile_id(profile)
        cached = self._profile_state_cache.get(pid)
        if cached:
            return cached
        state = effective_state(profile)
        self._profile_state_cache[pid] = state
        return state
    ...
    def selected_profile_obj(self) -> dict | None:
        if self.selected_profile == "all":
            return None
        for profile in self.profiles:
            if profile_id(profile) == self.selected_profile:
                return profile
        return None
    ...
    def active_desktop_home(self) -> str:
        return str(self.active_desktop_marker().get("codexHome") or "")
    ...
    def select_profile(self, pid: str) -> None:
        ...
        self.status_var.set("Selected all profiles" if pid == "all" else f"Selected {self.selected_profile_obj().get('name', pid) if self.selected_profile_obj() else pid}")
        self._update_buttons()
        self._refresh_account_selection_styles()
        self._render_details()
    ...
    def reload_profiles(self) -> None:
        self.profiles = load_profiles()
        if self.selected_profile != "all" and self.selected_profile not in {profile_id(profile) for profile in self.profiles}:
            self.selected_profile = "all"
        self.log("Reloaded profiles.")
        self.render()
    ...
    def next_account_home(self) -> Path:
        index = self.next_account_index()
        while True:
            candidate = DEFAULT_ACCOUNTS_ROOT / f"account-{index}"
            if not candidate.exists() and str(candidate) not in {str(profile.get("codexHome", "")) for profile in self.profiles}:
                return candidate
            index += 1
    ...
    def on_sort_changed(self) -> None:
        ...
        if mode not in SORT_CHOICES:
            ...
            return
        self.settings["sortMode"] = mode
        save_settings(self.settings)
        self.render()

    def on_card_template_changed(self) -> None:
        template = self.card_template_var.get()
        if template not in CARD_TEMPLATE_CHOICES:
            self.card_template_var.set("Balanced")
            return
        self.settings["cardTemplate"] = template
        save_settings(self.settings)
        self._render_accounts()
    ...
    def edit_selected_account(self) -> None:
        ...
        if profile is not None:
            self.add_account_dialog(profile)
    ...
    def rename_selected_account(self) -> None:
        ...
        def save() -> None:
            ...
            self.log(f"Renamed account to {name}.")
            dialog.destroy()
            self.render()
    ...
    def delete_selected_account(self) -> None:
        ...
        if profile is None:
            return
        index = self.profile_index(profile)
        if index < 0:
            return
        ...
        self.selected_profile = "all"
        self.save_current_profiles()
        self.log(f"Deleted profile entry for {name}.")
        self.render()
    ...
    def move_selected_account(self, delta: int) -> None:
        ...
        if index < 0 or target < 0 or target >= len(self.profiles):
            return
        self.profiles[index], self.profiles[target] = self.profiles[target], self.profiles[index]
        self.save_current_profiles()
        self.log(f"Moved {profile.get('name', 'Account')} {'up' if delta < 0 else 'down'}.")
        self.render()

    def add_account_dialog(self, existing: dict | None = None) -> None:
        index = self.next_account_index()
        home = self.next_account_home()
        editing = existing is not None
        existing_index = self.profile_index(existing)
        ...
        workspace_var = tk.StringVar(value=str(existing.get("workspace", DEFAULT_WORKSPACE)) if existing else str(DEFAULT_WORKSPACE))
        ...
        def provider_defaults(_event: tk.Event | None = None) -> None:
            ...
            if provider == "claude":
                ...
                home_var.set(str(provider_account_home("claude", CLAUDE_CLI_HOME)))
            elif provider == "cursor":
                ...
                if home_var.get().strip() in {str(CLAUDE_CLI_HOME), str(CLAUDE_ROAMING_HOME), str(CURSOR_ROAMING_HOME), str(ANTIGRAVITY_ROAMING_HOME)}:
        ...
        def create() -> None:
            ...
            profile = normalize_profile(payload, len(self.profiles))
            try:
                ensure_profile_home(profile)
            except Exception as error:
            ...
            self.selected_profile = profile_id(profile)
            self.search_var.set("")
            self.save_current_profiles()
            dialog.destroy()
            self.log(f"{'Updated' if editing else 'Added'} {provider_label(profile)} account: {name}.")
            self.render()
